        ...
        for s in stmts:
            if s[0] == 'exp' or s[0] == 'print' or s[0] == 'error':
                ...
                if (s[2] != []):
                    ...
                    s2 = s[2]
                dStmts.append(('call', desugarExp(s[1]), s2))
            elif s[0] == 'doMulti':
                result = []
                s1 = desugarExp(s[1])
                s2 = desugarExp(s[2])
                if (s[3] != []):
                    ...
                    s3 = s[3]
                result.append(('def', '#doMulti', ('int-lit', 0)))
                lessthan = ('<', ('var', '#doMulti'), s1)
                calling = []
                calling.append(('exp',('call', s2, s3),[]))
                calling.append(('asgn', '#doMulti', ('+', ('var', '#doMulti'), ('int-lit', 1))))
                whileloop = [('while', lessthan, calling)]
                result.extend(desugarStmts(whileloop))
                dStmts.extend(result)
            else:
                dStmts.append(s)
        return dStmts
